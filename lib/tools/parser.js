...
const extractToolCall = (text) => {
  ...
  while ((match = tagRegex.exec(text))) {
    ...
    if (name === "thinking" || name === "analysis" || name === "attempt_completion" || name === "title_name") {
      continue;
    }
    ...
    if (name === "tool_call" || name === "tool") {
      ...
      if (!nameMatch) {
        ...
        const inlineName = String(inlineNameMatch[1] || "").trim().toLowerCase();
        if (inlineName === "thinking" || inlineName === "analysis" || inlineName === "attempt_completion" || inlineName === "title_name") {
          continue;
        }
      ...
      const toolName = String(nameMatch[1] || "").trim().toLowerCase();
      if (toolName === "thinking" || toolName === "analysis" || toolName === "attempt_completion" || toolName === "title_name") {
        continue;
      }
  ...
  if (looseMatch) {
    const name = String(looseMatch[1] || "").trim().toLowerCase();
    if (allowedTools.has(name)) {
